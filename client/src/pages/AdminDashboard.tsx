...
import {
  LogOut, IndianRupee, ShoppingCart, Users, Package, TrendingUp,
  ...
  Target, Filter, Calendar as CalendarIcon, PieChart as PieChartIcon, Briefcase, Rocket
} from "lucide-react";
...
function StartupInquiriesTab() {
  ...
  const startupInquiries = leads?.filter((lead: any) => lead.kind === "startup") || [];
    ...
    return (
      <Card>
        <CardContent className="p-8 text-center">
          ...
          <p className="text-muted-foreground">Loading startup inquiries...</p>
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Student startup launch inquiries and contact submissions ({startupInquiries.length} total)</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
          {startupInquiries.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground">No startup inquiries yet</p>
              <p className="text-sm text-muted-foreground">When students submit startup launch forms, they will appear here</p>
            </div>
          ) : (
                ...
                <div key={inquiry.id} className="border rounded-lg p-4 hover:bg-muted/50 transition-colors">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <Badge variant="outline" className="bg-orange-50 text-orange-700 border-orange-200">
                      ...
                      {inquiry.metadata?.stage || "Inquiry"}
                    </Badge>
                  </div>
                  <p className="text-sm mb-3 text-foreground">{inquiry.message}</p>
                  <div className="flex flex-wrap gap-3 text-xs text-muted-foreground">
                    <span className="flex items-center gap-1">
                      <IndianRupee className="h-3 w-3" />
                      Funding Needed: ₹{inquiry.metadata?.fundingNeeded || "N/A"}
                    </span>
                    <span>Submitted: {format(new Date(inquiry.createdAt), "MMM dd, yyyy")}</span>
                  </div>
...
export default function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-zinc-50 via-zinc-50 to-zinc-100 dark:from-zinc-950 dark:via-zinc-950 dark:to-zinc-900">
      {/* Top Navigation Bar */}
      ...
      <main className="mx-auto px-4 lg:px-8 py-6 max-w-[1600px]">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          ...
          <Card className="hover:shadow-lg transition-all duration-200 border-l-4 border-l-zinc-500">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <CardContent>
              ...
              <div className="flex items-center text-xs text-muted-foreground mt-2">
                <BarChart3 className="w-3 h-3 mr-1 text-zinc-600" />
                      ...
                      {analytics?.weeklyTrend && analytics.weeklyTrend.length > 0 ? (
                        <ResponsiveContainer width="100%" height={300}>
                          <LineChart data={analytics.weeklyTrend}>
                            ...
                            <Line yAxisId="right" type="monotone" dataKey="orders" stroke="#71717a" strokeWidth={2} name="Orders" />
                            <Line yAxisId="right" type="monotone" dataKey="leads" stroke="#d4d4d8" strokeWidth={2} name="Leads" />
                          </LineChart>
                        </ResponsiveContainer>
                      ) : (
                      ...
                      {analytics && (analytics.completedOrders > 0 || analytics.pendingOrders > 0 || analytics.processingOrders > 0 || analytics.cancelledOrders > 0) ? (
                        <ResponsiveContainer width="100%" height={250}>
                          <PieChart>
                            <Pie
                              data={[
                                { name: 'Completed', value: analytics?.completedOrders || 0, color: '#f97316' },
                                { name: 'Pending', value: analytics?.pendingOrders || 0, color: '#71717a' },
                                { name: 'Processing', value: analytics?.processingOrders || 0, color: '#a1a1aa' },
                ...
                <div className="grid gap-6 md:grid-cols-3">
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <div className="p-3 bg-gradient-to-br from-orange-50 to-orange-100 dark:from-orange-900/20 dark:to-orange-900/10 rounded-lg">
